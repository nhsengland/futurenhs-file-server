...
{
    internal sealed class RedirectToFileStoreRequestHandler : WopiRequestHandler
    {
        private readonly File _file;

        private RedirectToFileStoreRequestHandler(File file, string accessToken)
        ...
        internal static RedirectToFileStoreRequestHandler With(File file, string accessToken) => new RedirectToFileStoreRequestHandler(file, accessToken);
        ...
        protected override async Task HandleAsyncImpl(HttpContext httpContext, CancellationToken cancellationToken)
        {
            ...
            var fileContentService = httpContext.RequestServices.GetRequiredService<IFileContentService>();

            var uri = await fileContentService.GenerateEphemeralDownloadLink(fileMetadata, cancellationToken);
