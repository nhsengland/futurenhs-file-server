...
namespace FutureNHS.WOPIHost.WOPIRequests
{
        ...
        : WopiRequestHandler
    {
        ...
        private CheckFileInfoWopiRequestHandler(File file, string accessToken, Features? features) 
            : base(accessToken, false, true) 
        {
        ...
        internal static CheckFileInfoWopiRequestHandler With(File file, string accessToken, Features features) => new(file, accessToken, features);
        ...
        protected override async Task HandleAsyncImpl(HttpContext httpContext, CancellationToken cancellationToken)
        {
            ...
            if (FileStatus.Verified != fileMetadata.FileStatus) throw new ApplicationException($"The status of the file '{fileMetadata.FileStatus?.ToString() ?? "null"}' does not indicate it is yet safe to be shared with users.");
            ...
            //        Not sure how this works. Collabora in view mode has a download option but when clicking it this url isn't hit, 
            //        yet the file is sent to the browser.   Figure out what is going on and how we are misusing this property
            var downloadUrl = $"gateway/wopi/host/files/{_file.Id}/contents?access_token={AccessToken}&ephemeral_redirect=true";
