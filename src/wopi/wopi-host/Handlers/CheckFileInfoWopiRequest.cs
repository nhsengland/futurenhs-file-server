...
namespace FutureNHS.WOPIHost.Handlers
{
        ...
        : WopiRequest
    {
        ...
        protected override async Task HandleAsyncImpl(HttpContext httpContext, CancellationToken cancellationToken)
        {
            ...
            var userDelegatedBlobStoreEndpoint = await fileRepository.GeneratePrivateEphemeralDownloadLink(fileMetadata, cancellationToken);
            ...
            responseBody.OwnerId = fileMetadata.Owner;                               // A string that uniquely identifies the owner of the file. In most cases, the user who uploaded or created the file should be considered the owner.
            responseBody.Size = fileMetadata.SizeInBytes;                            // The size of the file in bytes, expressed as a long, a 64-bit signed integer.
            responseBody.UserId = "richard@iprogrammer.co.uk";                       // A string value uniquely identifying the user currently accessing the file.
            responseBody.Version = fileMetadata.Version;                             // The current version of the file based on the server’s file version schema, as a string. This value must change when the file changes, and version values must never repeat for a given file.
            ...
            responseBody.SupportsUpdate = true; // supportsUpdate;                   // A Boolean value that indicates that the host supports the PutFile and PutRelativeFile WOPI operations
            ...
            responseBody.UserFriendlyName = "Richard Ashman";                        // A string that is the name of the user, suitable for displaying in UI. Eg when making comments in a file (can we inject avatar some how)?

            // User permissions

            responseBody.ReadOnly = false; // !supportsUpdate;                       // A Boolean value that indicates that, for this user, the file cannot be changed.
            ...
            responseBody.UserCanWrite = true; // supportsUpdate;                     // A Boolean value that indicates that the user has permission to alter the file. Setting this to true tells the WOPI client that it can call PutFile on behalf of the user.

            // File URLs

            var fileUrl = userDelegatedBlobStoreEndpoint.AbsoluteUri;

            // TODO - Needs to be the most recent file version.  Test if has to be absolute url or if relative works
            var downloadUrl = $"gateway/media/files/{_file.Id}/contents?access_token={AccessToken}&ephemeral_redirect=true";
            ...
            responseBody.LastModifiedTime = fileMetadata.LastWriteTime.ToIso8601();  // A string that represents the last time that the file was modified. This time must always be a must be a UTC time, and must be formatted in ISO 8601 round-trip format. For example, "2009-06-15T13:45:30.0000000Z".
            ...
            await System.Text.Json.JsonSerializer.SerializeAsync(httpContext.Response.Body, responseBody, cancellationToken: cancellationToken);
        }
    }
}
